class Node:
    
    def __init__(self,data,left,right):
        self.data = data
        self.left = None
        self.right = None
class Pair:
    def __init__(self,node,state):
        self.node = node
        self.state = state

def construct(arr):
    root=Node(arr[0],None,None)
    rtp=Pair(root,1);
    
    st=[]
    st.append(rtp);
    
    idx=0;
    n = len(arr)
    while(len(st)>0):
        top=st[-1];
        if top.state==1:
            idx+=1
            st[-1].state+=1
            if(arr[idx]!=-1):
                top.node.left = Node(arr[idx], None, None);
                lp = Pair(top.node.left, 1);
                st.append(lp);
            else:
                top.node.left=None;
        elif top.state==2:
            idx+=1
            st[-1].state+=1
            if(arr[idx]!=-1):
                top.node.right =  Node(arr[idx], None, None);
                rp =  Pair(top.node.right, 1);
                st.append(rp);
            else:
                top.node.right=None;
        else:
            st.pop()
    return root;

def height(node):
    if(node == None):
        return -1
    else:
        max(height(node.left),height(node.right))+1
    return ans

def tilt(node):
    if node == None:
        return [0 ,0]
    
    all_lt_sum, tlf = tilt(node.left)
    all_rt_sum, trf = tilt(node.right)
    all_sum = all_lt_sum + all_rt_sum + node.data
    tilt_sum = tlf + trf + abs(all_lt_sum - all_rt_sum)
    return [all_sum, tilt_sum]

n = int(input())
st = input()
arr = [0]*n
arr = list(map(int,st.replace("n","-1").split(" ")));


root = construct(arr)
all_sum, tile_sum = tilt(root)
print(tile_sum)